...
const toPoint = (grade) => GRADE_POINTS[grade?.trim()] ?? 0.00;
...
function computeSummary(rows) {
  ...
  for (const s of Object.values(semMap)) {
    s.gpa = s.semesterCredits > 0 ? parseFloat((s.semesterPoints / s.semesterCredits).toFixed(2)) : 0;
  }
...
exports.getByStudentNumber = async (req, res, next) => {
  try {
...
exports.getSemesters = async (_req, res, next) => {
  try {
    const rows = await db.query(
...
exports.uploadResult = async (req, res, next) => {
  try {
    ...
    const student = await db.queryOne(`SELECT id FROM users WHERE student_number = $1`, [student_number]);
    ...
    const gp = grade_point ?? toPoint(letter_grade);
    let result;
    ...
    if (existing) {
      const rows = await db.query(
        ...
        [letter_grade, gp, course_title, credit_hours, batch_section, status, existing.id]
      );
      result = rows[0];
    } else {
      const rows = await db.query(
        ...
        [student?.id || null, student_number, semester_code, semester_name,
         ...
         letter_grade, gp, status, is_published, req.user.id]
      );
      result = rows[0];
    }
...
exports.bulkSave = async (req, res, next) => {
  try {
    ...
    for (const row of resultRows) {
      const gp = toPoint(row.letter_grade);
      const existing = await db.queryOne(
        `SELECT id FROM results WHERE student_number=$1 AND semester_code=$2 AND course_code=$3`,
...
exports.publishResult = async (req, res, next) => {
  try {
    const rows = await db.query(
      `UPDATE results SET is_published=TRUE, publish_date=NOW() WHERE id=$1 RETURNING *`,
      [req.params.id]
    );
...
exports.publishSemester = async (req, res, next) => {
  try {
  ...
  try {
    ...
    let sql = `
      SELECT
        ...
        ROUND(SUM(r.credit_hours * r.grade_point) / NULLIF(SUM(r.credit_hours), 0), 2) AS cgpa
      FROM users u
      LEFT JOIN results r ON r.student_number = u.student_number AND r.is_published = TRUE
